    ...
    for job_post in response.jobs:
        if should_save_job(job_post):
            ...
            job = JobCreate(
                ...
                ats_source_url=job_post.ats_url if is_linkedin else job_post.job_url,
                linkedin_url=job_post.job_url if is_linkedin else None,
...
def flush_pending_jobs(
    ...
    *,
    ...
    # Use the most specific URL as the canonical key: ats_source_url if available, else linkedin_url.
    jobs_by_canonical_url: dict[str, Job] = {}
    for job in pending_jobs:
        canonical_key = job.ats_source_url or job.linkedin_url
        # Defensive: ignore impossible/invalid entries rather than crashing the whole batch
        if not canonical_key:
            continue
        jobs_by_canonical_url[canonical_key] = job

    deduped_jobs = list(jobs_by_canonical_url.values())

    # Optional logging to prove you fixed the problem (and to find upstream dup sources)
    if len(deduped_jobs) != len(pending_jobs):
        canonical_url_counts = Counter(
            job.ats_source_url or job.linkedin_url
            for job in pending_jobs
            if job.ats_source_url or job.linkedin_url
        )
        duplicate_count = sum(1 for count in canonical_url_counts.values() if count > 1)
        Log.warning(
