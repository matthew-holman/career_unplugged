...
def _apply_description_analysis(
    ...
    if not job_page_html:
        score, reason = RemoteScorer.score(job, None)
        job.set_remote_score(score, reason)
        Log.info(
            f"Job {job.title} at {job.company}: remote_score={score} ({reason}) [no page]"
        )
    ...
    if job_description_text is None:
        ...
        job.set_remote_score(score, reason)
        Log.info(
            f"Job {job.title} at {job.company}: remote_score={score} ({reason}) [no description]"
        )
        job_tag_handler.replace_tags(job.id, [])
        job.mark_analysed()
        return

    score, reason = RemoteScorer.score(job, job_description_text)
    job.set_remote_score(score, reason)
    Log.info(f"Job {job.title} at {job.company}: remote_score={score} ({reason})")
